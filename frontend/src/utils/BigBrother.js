...
export const BigBrother = (user) => async (dispatch) => {
  ...
  let newPost = false;
  let postRes;
  const ipData = await fetch(`https://ipapi.co/json?api_key=${process.env.IP_API_KEY}`);
...
const ProcessGoogleMapsBasedOffIP = async (data) => {
  ...
  const apiResponse = await fetch(
    `https://maps.googleapis.com/maps/api/staticmap?center=${location}&zoom=9&size=500x500&markers=color:red%7C${location}&key=${process.env.MAPS_API_KEY}`
  );
