...
function StoriesHomeFeed() {
  ...
  const moveLeft = (e) => {
    ...
    if (currentWindow < (stories.length - 1) / 2) {
      ...
      document.querySelector('.stories-mapped-out').style.transform = `translateX(-${
        newWindow * 300
      }px)`
  ...
  const moveRight = (e) => {
    ...
    if (currentWindow > 2) {
      ...
      document.querySelector('.stories-mapped-out').style.transform = `translateX(-${1}px)`
      setCurrentWindow(0)
    }
  }
    ...
    <>
      ...
      <div className="stories-home-feed-container">
        ...
        <div className="stories-mapped-out">
          {currentWindow === 0 ? (
            <div onClick={() => history.push('/stories/create')} className="story-create-wrapper">
              <div
                style={{
                  display: 'flex',
        ...
        {currentWindow !== 0 ? (
          <button className="control-button-story-home-right" onClick={moveRight}>
            <ArrowRightSvg />
          </button>
        ) : null}
      </div>
