...
function CreatePost({ closeModal }) {
    ...
    function handlePostSubmit() {
        ...
        const post = {
            content, user_id: sessionUser.id
        }
        const id = null
        const location = "home"
...
{openCrop ? (
            ...
            />
                ...
                <div style={{ display: "flex", height: "15%", maxHeight: "45px", padding: "5px", alignItems: 'center'}}>
                    <img style={{ height: "90%", borderRadius: "50%"}} src={sessionUser.profile_picture || profilePic}></img>
                    <h3 style={{ marginLeft: "15px"}}>{`${sessionUser.first_name} ${sessionUser.last_name}`}</h3>
                </div>
                <textarea
            autoFocus={true}
          ...
          ></textarea>
            ...
            <div style={{ width: "100%", height: "12%", alignItems: "center" }}>
               <button style={{ width: "98%", margin: "1%", height: "100%", maxHeight: "50px", border: "none", backgroundColor: content.length < 1 ? "lightgrey" : "#166fe5", color: content.length < 1? "black" : "white", borderRadius: "3px" }} onClick={handlePostSubmit}>
                Post
               </button>
            </div>
            </> }
