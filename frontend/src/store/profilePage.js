...
export const profilePage = (id, noPosts, noFriends ) => async (dispatch) => {
    ...
    if (noPosts) return Promise.all([dispatch(setCurrentProfile(userData.user)), dispatch(receiveFriends(friendsData))])
    const postRes = await csrfFetch(`/api/posts/${id}`)
    const postData = await postRes.json() 
    Promise.all([dispatch(receivePosts(postData)), dispatch(setCurrentProfile(userData.user)), dispatch(receiveFriends(friendsData))])
}
